...
async function processJob(job: Job) {
  ...
  log.info(`Processing automation job: ${job.id}`, {
  ...
  try {
    ...
    for (const action of actions) {
      log.debug(`Executing action ${actionIndex}: ${action.type}`, {
        jobId: job.id,
      });
...
async function executeAction(
  ...
  switch (action.type) {
    ...
    case "EMAIL":
      ...
      log.info(
        `Sending mock email to user ${userId} with template ${action.templateId}`,
      );
    ...
    case "COMMUNITY_JOIN":
      ...
      log.info(
        `Automatically joining user ${userId} to community ${action.communityId}`,
      );
      return { success: true };
...
async function init() {
  const worker = new Worker(QUEUE_NAME, processJob, {
    ...
    concurrency: parseInt(
      process.env.AUTOMATION_WORKER_CONCURRENCY || "10",
      10,
    ),
  });
...
const healthServer = http.createServer((req, res) => {
  if (req.url === "/health") {
    ...
    res.end(
      JSON.stringify({ status: "healthy", service: "automation-worker" }),
    );
  } else {
    res.writeHead(404);
    res.end();
