...
exports.countryService = {
     ...
     * Get all countries
     */
    GetAllCountries: (call, callback) => {
        try {
            logging_1.log.info('GetAllCountries called');
            const response = {
            ...
            logging_1.log.error('Error in GetAllCountries', { error: error.message });
            callback(error);
        }
    },
    /**
     ...
     */
    GetCountryDetails: (call, callback) => {
        try {
            const { code } = call.request;
            logging_1.log.info('GetCountryDetails called', { code });
            ...
            logging_1.log.error('Error in GetCountryDetails', { error: error.message });
            callback(error);
        }
    },
};
