...
class EntityClient {
  ...
  async getEntityDetails(id: string): Promise<any> {
    return new Promise((resolve, reject) => {
      this.client.GetEntityDetails(
        { id },
        (error: grpc.ServiceError | null, response: any) => {
          if (error) {
            log.error("gRPC GetEntityDetails error", {
              id,
              error: error.message,
            });
            reject(error);
  ...
  async editEntityLogo(params: { entityId: string; logo: string }): Promise<{
    success: boolean;
    message: string;
    logo: string;
  }> {
    return new Promise((resolve, reject) => {
      this.client.EditEntityLogo(
        params,
        (error: grpc.ServiceError | null, response: any) => {
          if (error) {
            log.error("gRPC EditEntityLogo error", {
  ...
  async editEntityProfile(params: { name: string; entityId: string }): Promise<{
    success: boolean;
    message: string;
  }> {
    return new Promise((resolve, reject) => {
      this.client.EditEntityProfile(
        params,
        (error: grpc.ServiceError | null, response: any) => {
          if (error) {
            log.error("gRPC EditEntityProfile error", {
              entityId: params.entityId,
              error: error.message,
            });
            reject(error);
